## kubectl plugin svcat get plans

List plans, optionally filtered by name or class
...
```
kubectl plugin svcat get plans [flags]
```
...
```
  -c, --class string    Filter plans based on class. When --uuid is specified, the class name is interpreted as a uuid.
  -h, --help            help for plans
  -o, --output string   The output format to use. Valid options are table, json or yaml. If not present, defaults to table
  -u, --uuid string     Whether or not to get the plan by UUID (the default is by name)
```
