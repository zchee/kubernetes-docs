...
List bindings, optionally filtered by name
...
```
kubectl plugin svcat get bindings [flags]
```
...
```
      --all-namespaces string   If present, list the requested object(s) across all namespaces. Namespace in current context is ignored even if specified with --namespace
